...
# 真人多点簇的峰值在 4 点。
#
# **簇长在 4 点封顶**（真人尾部 5~11 点的那 11.1% 不采用）：首击一旦点掉
# 奖励页，后续追加击就落在新出现的界面上，而安全区域是按奖励页算的——
# 在新界面上那个坐标可能正好是「再来一局」「进入下一关」之类的按钮。
# 真人误触了会自己退回来，脚本不会，且长簇误触的代价随任务链路放大。
# 封顶 4 点保留 88.9% 的真人簇（含多点簇峰值），把最长暴露窗口从 2.20s
# 压到 0.66s；期望追加击 1.33→0.89，单击占比 57.2%→64.3%。
MULTI_CLICK_SIZES = (1, 2, 3, 4)          # 一次手势的总点击数取值（4 点封顶，见上）
MULTI_CLICK_WEIGHTS = (258, 18, 35, 90)   # 对应权重＝真人簇长直方图的前四档
